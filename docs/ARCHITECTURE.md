...
- `scripts/build_linux.sh`: Linux x86_64 one-folder package and installer tarball.
...
PyInstaller uses `--onedir` for faster repeated launches instead of unpacking a one-file runtime every time. The workflow builds on `ubuntu-22.04` x86_64, installs Tk and packaging prerequisites, creates a PyInstaller one-folder bundle, adds a desktop-entry installer, and produces a compressed tarball.
